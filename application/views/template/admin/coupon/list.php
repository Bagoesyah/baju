...
<div class="content-wrapper" ng-app="couponModule" ng-controller="couponController as mc">
        ...
        <div class="box">
            <div class="box-header">
                <?php if (isset($_created) == 1): ?>
                    <a href="<?php echo site_url('admin/coupon/add'); ?>" class="btn btn-primary btn-sm">Add Coupon</a>
                <?php endif; ?>
            ...
            <div class="box-body table-responsive">
                ...
                <thead>
                    <tr>
                        ...
                        <th st-sort="code">Coupon</th>
                        <th st-sort="discount">Discount</th>
                        <th st-sort="num_type">Type</th>
                        <th width="160" st-sort="expired_at">Expired At</th>
                        <th width="160" st-sort="created_at">Created At</th>
                        <th width="110" rowspan="2" class="th-top">Action</th>
                    </tr>
                    <tr>
                        <th><input st-search="code" placeholder="Search.." class="input-sm form-control"></th>
                        <th><input st-search="discount" placeholder="Search.." class="input-sm form-control"></th>
                        <th></th>
<th></th><th></th>
                    </tr>
                </thead>
                <tbody ng-show="!mc.isLoading" ng-cloak>
                    <tr ng-repeat="row in mc.displayed">
                        ...
                        <td>{{row.code}}</td>
                        <td>{{row.discount}}</td>
                        <td>{{row.num_type == 'v' ? 'Value' : 'Percentage'}}</td>
                        ...
                        <td class="td-action">
                            ...
                            <ul class="dropdown-menu ul-action" role="menu">
                                <?php if (isset($_updated) == 1): ?>
                                    <li>
                                        <a href="<?php echo site_url('admin/coupon/edit/{{row.id}}'); ?>">
            ...
            <table class="table table-hover">
                <tbody>
                    ...
                    <tr>
                        <th>Discount</th>
                        <td width="10">:</td>
                        <td>{{ selected.item.discount }}</td>
                    </tr>

                    <tr>
                        <th>Type</th>
                        <td width="10">:</td>
                        <td>{{ selected.item.num_type == 'v' ? 'Value' : 'Percentage' }}</td>
                    </tr>

                    <tr>
                        <th>Expired At</th>
                        <td width="10">:</td>
                        <td>{{ selected.item.expired_at }}</td>
                    </tr>
...
<script>
var app = angular.module('couponModule', ['smart-table', 'oitozero.ngSweetAlert', 'ui.bootstrap']);
...
app.factory('Resource', ['$q', '$filter', '$timeout', '$http', function ($q, $filter, $timeout, $http) {
    ...
    function getPage(start, number, params) {
        ...
        var getData = $http.get("<?php echo base_url('admin/coupon/get_data'); ?>")
...
app.controller('couponController', ['Resource', '$scope', 'SweetAlert', '$http', '$uibModal',
    ...
    $scope.removeItem = function removeItem(row) {
        ...
        function (isConfirm) {
            if (isConfirm) {
                ...
                if (index !== -1) {
                    $http.delete("<?php echo base_url('admin/coupon/delete'); ?>" + "/" + row.id);
