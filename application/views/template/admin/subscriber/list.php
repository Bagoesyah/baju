...
# @Last modified time: 2017-04-21T16:48:31+07:00
?>

<!-- Content Wrapper. Contains page content -->
<div class="content-wrapper" ng-app="subscriberModule" ng-controller="subscriberController as mc">
        ...
        <div class="box">
            <div class="box-header">
                <?php if (isset($_created) == 1): ?>
                    <a href="" class="btn btn-primary btn-sm" ng-click="openBlastEmail()">Blast Email</a>
                <?php endif; ?>
...
<script type="text/ng-template" id="myBlastEmailModalContent.html">
    <div class="modal-header">
        <h3 class="modal-title">Blast Email</h3>
    </div>
    <div class="modal-body">
        <div class="table-responsive">
            <table class="table table-hover">
                <tbody>
                    <tr>
                        ...
                        <td width="10">:</td>
                        <td>
                            <select name="blast_type" id="blast_type" class="form-control" ng-model="selectedType">
                                <option value="product">New Products</option>
                                <option value="promo">New Promotions</option>
                                <option value="both">New Products &amp; Promotions</option>
                            </select>
                        </td>
                    </tr>
                </tbody>
    ...
    <div class="modal-footer">
        <button class="btn btn-success" type="button" ng-click="blastNow()">Run</button>
        <button class="btn btn-warning" type="button" ng-click="cancel()">Close</button>
    </div>
</script>
</div><!-- /.content-wrapper -->
...
<script>
var app = angular.module('subscriberModule', ['smart-table', 'oitozero.ngSweetAlert', 'ui.bootstrap']);
...
app.factory('Resource', ['$q', '$filter', '$timeout', '$http', function ($q, $filter, $timeout, $http) {
    ...
    function getPage(start, number, params) {
        ...
        var getData = $http.get("<?php echo base_url('admin/subscriber/get_data'); ?>")
...
app.controller('subscriberController', ['Resource', '$scope', 'SweetAlert', '$http', '$uibModal',
function (service, $scope, SweetAlert, $http, $uibModal) {
    ...
    $scope.openBlastEmail = function() {
        var modalInstance = $uibModal.open({
            templateUrl: 'myBlastEmailModalContent.html',
            controller: 'ModalBlastEmailInstanceCtrl',
            resolve: {
                items: function () {
                    return $scope.items;
                }
    ...
    $scope.removeItem = function removeItem(row) {
        ...
        function (isConfirm) {
            if (isConfirm) {
                ...
                if (index !== -1) {
                    $http.delete("<?php echo base_url('admin/subscriber/delete'); ?>" + "/" + row.id);
...
app.controller('ModalBlastEmailInstanceCtrl', function ($scope, $uibModalInstance, $http, SweetAlert) {

    $scope.cancel = function () {
        $uibModalInstance.dismiss('cancel');
    };

    $scope.blastNow = function blastNow() {
        $uibModalInstance.dismiss('cancel');
        SweetAlert.swal({
            title: "Are you sure?",
            text: "This will send an email to all subscribers!",
            type: "warning",
            showCancelButton: true,
            confirmButtonColor: "#DD6B55", 
        ...
        function (isConfirm) {
            if (isConfirm) {
                ...
                $http({
                    method: 'POST',
                    url: "<?php echo base_url('api/subscriber/blast'); ?>",
                    data: {
                ...
                //$http.post("<?php echo base_url('admin/subscriber/blast'); ?>");
                $scope.$broadcast('refreshData');
                SweetAlert.swal("Success!", "Sending email is in progress.", "success");
            } else {
                SweetAlert.swal("Cancelled", "Sending email cancelled", "error");
            }
        });
    }
});
